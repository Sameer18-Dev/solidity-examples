...
describe("Sample Apps / Vault", async function () {
  ...
  const TokenContract = await ethers.getContractFactory(
    "contracts/sample_apps/10_vault/Vault.sol:AssetToken",
    deployer
  );
  ...
  const VaultContract = await ethers.getContractFactory(
    "contracts/sample_apps/10_vault/Vault.sol:Vault",
    deployer
  );
  ...
  it("is initialed deposit into the blank pool", async function () {
    ...
    expect(await tokenContract.balanceOf(vaultContract.address)).to.eq(
