...
describe("Sample Apps / ETH Game", function () {
  ...
  beforeEach(async () => {
    ...
    const Contract = await ethers.getContractFactory(
      "contracts/sample_apps/08_eth_game/EthGame.sol:EthGame"
    );
  ...
  it("Should prevent forcefully sending eth the balance", async function () {
    await expect(contract.deposit({ value: ethers.utils.parseEther("1.5") })).to.be.revertedWith(
      "You can only send 1 Ether"
    );
  });
